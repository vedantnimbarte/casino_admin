...
import { updateGameType } from 'store/thunk/configuration/gameType.thunk';

function UpdateGameType({ dispatch, isMobileDevice, openModal, setOpenModal, theme, gameGroupSchema, gameTypeIndex, gameType }) {
    const formik = useFormik({
        initialValues: {
            name: gameType.data[gameTypeIndex].GAMEGROUP_NAME || '',
            description: gameType.data[gameTypeIndex].DESCRIPTION || '',
            id: gameType.data[gameTypeIndex].GAMEGROUP_ID
        },
        validationSchema: gameGroupSchema,
        onSubmit: (values) => {
            dispatch(updateGameType(values));
